# This file was auto-generated by Fern from our API Definition.

from __future__ import annotations

import typing

import pydantic
from ..core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel, update_forward_refs
from .time_series_feature_model import TimeSeriesFeatureModel


class TimeSeriesFeatureCollectionModel(UniversalBaseModel):
    """
    GeoJSON FeatureCollection for time-series features.
    """

    bbox: typing.Optional[typing.List[typing.Optional[typing.Any]]] = None
    type: typing.Literal["FeatureCollection"] = "FeatureCollection"
    features: typing.List[TimeSeriesFeatureModel]

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


update_forward_refs(TimeSeriesFeatureCollectionModel)
